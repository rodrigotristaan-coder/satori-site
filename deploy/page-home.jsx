...
function BrandReel() {
  ...
  const total = 5;
  useEffect(() => {
    const id = setInterval(() => setScene((s) => (s + 1) % total), 4200);
...
function ReelLineScene() {
  return (
    ...
    >
      <svg viewBox="0 0 800 200" style={{ width: "80%", maxWidth: 800 }}>
        <defs>
        ...
        <path
          d="M 20 100 Q 200 20, 400 100 T 780 100"
          stroke="url(#reelLineGrad)"
          strokeWidth="2.5"
          fill="none"
          strokeDasharray="800"
          style={{ animation: "reelDrawLine 3.4s ease-out both" }}
        />
        {[100, 280, 400, 540, 720].map((cx, i) => (
          <circle
            key={cx}
            cx={cx}
            cy={100 + (i % 2 === 0 ? -15 : 15)}
            ...
            style={{
              opacity: 0,
              animation: `reelFadeIn .6s ease ${0.5 + i * 0.35}s both`
            }}
          />
        ))}
      </svg>
      <div
        style={{
          ...
          color: SATORI.GOLD,
          opacity: 0,
          animation: "reelFadeIn 1s ease 1.6s both"
        }}
      >
        RUTA DE CRECIMIENTO
      </div>
...
function App() {
  ...
  useEffect(() => {
    try {
      if (sessionStorage.getItem("satori_seen_welcome")) setWelcomeDone(true);
    } catch (e) {}
  }, []);
  ...
  return (
    <main style={{ position: "relative", minHeight: "100vh", background: SATORI.CREAM }}>
      {!welcomeDone && (
        <WelcomeAnimation duration={3600} onComplete={() => setWelcomeDone(true)} />
      )}
