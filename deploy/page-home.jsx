  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="manifesto-logo-typewriter">
            <SatoriMark height={247} variant="gold" />
      ...
      <style>{`
        @media (max-width: 900px) {
          .manifesto-logo img { height: 171px !important; }
        }
        @media (max-width: 560px) {
          .manifesto-logo img { height: 114px !important; }
        }
        @media (max-width: 380px) {
          .manifesto-logo img { height: 90px !important; }
        }
        /* Z Fold (~280px folded) */
        @media (max-width: 320px) {
          .manifesto-logo img { height: 72px !important; }
        }
...
function SatoriGlobe() {
  ...
  const dragState = useRef({ active: false, lastX: 0, lastY: 0, idleAt: 0 });
  ...
  // Rotación automática lenta (pausa cuando se arrastra o recién se arrastró)
  useEffect(() => {
    if (!d3Ready) return;
    let raf;
    ...
    const tick = (t) => {
      const dt = t - last;
      last = t;
      const idleFor = t - dragState.current.idleAt;
      if (!dragState.current.active && idleFor > 1200) {
        setRotation((r) => (r + dt * 0.006) % 360); // ~6 deg/sec
      }
  ...
  // Helper: ¿el puntero está DENTRO del disco del globo?
  const pointerInsideGlobe = (e) => {
    const svg = svgRef.current;
    if (!svg) return false;
    const rect = svg.getBoundingClientRect();
    if (rect.width === 0 || rect.height === 0) return false;
    // El SVG es cuadrado con viewBox 0..SIZE. El disco es radio RADIUS al centro.
    const px = ((e.clientX - rect.left) / rect.width) * SIZE;
    const py = ((e.clientY - rect.top) / rect.height) * SIZE;
    const dx = px - CENTER;
    const dy = py - CENTER;
    // Pequeño margen para que sea ergonómico en táctil
    const r = RADIUS + 6;
  ...
  const onPointerDown = (e) => {
    ...
    if (!pointerInsideGlobe(e)) return; // sólo gira si tocas el globo
  ...
  const onPointerMove = (e) => {
    ...
    setRotation((r) => (r + dx * 0.45) % 360);
    setTilt((t) => Math.max(-80, Math.min(80, t + dy * 0.35)));
  };
  const endDrag = (e) => {
    if (!dragState.current.active) return;
    ...
    >
      ...
      <style>{`
        ...
        @media (max-width: 860px) {
          .globe-wrap { max-width: 360px !important; }
        }
        @media (max-width: 560px) {
          .globe-wrap { max-width: 280px !important; }
        }
        @media (max-width: 380px) {
          .globe-wrap { max-width: 230px !important; }
        }
        @media (max-width: 320px) {
