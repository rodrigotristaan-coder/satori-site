...
function Enso({ size = 360, opacity = 0.92, spinning = true, color = "#8a8a8a" }) {
  // Recolor a black PNG: gris (gray, default), white (light), or original black
  const c = (color || "").toLowerCase();
  let filter = "none";
  if (c === "#f4f4f2" || c === "#fff" || c === "#ffffff") {
    filter = "invert(1)";
  } else if (c !== "#0e0e0e" && c !== "#000" && c !== "#000000") {
    // Map black silhouette to gris (mid gray) via filter chain
