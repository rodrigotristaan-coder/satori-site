...
// Accent color of the NEXT theme — for problema icons
    ...
    servicios: [
      {
        ...
        back: "Posts, reels y stories diseñados para posicionarte como el experto. Incluye estrategia mensual, diseño, copywriting con IA y reporte de métricas.",
      ...
      },
      {
        ...
        back: "Diseño profesional, copywriting estratégico, optimización móvil. Entrega en 7–10 días. La base que todo negocio serio necesita.",
      ...
      },
      {
        ...
        back: "Setup, segmentación avanzada, creativos, optimización semanal y reporte mensual. Ideal para escalar cuando ya tienes presencia.",
...
function AIWorkflow({ accent, card }: { accent: string; card: string }) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "1rem 1rem", gap: "0.4rem", backgroundColor: card }}>
      {steps.map((s, i) => (
        <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
          style={{ display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.7rem", fontFamily: "monospace", padding: "0.4rem 0.7rem", backgroundColor: `${accent}10`, color: accent, border: `1px solid ${accent}20`, borderRadius: 2 }}>
          <span style={{ opacity: 0.35, fontSize: "0.6rem", width: "1rem", flexShrink: 0 }}>{i + 1}.</span>
          {s}
          {i < 4 && <span style={{ marginLeft: "auto", opacity: 0.2, fontSize: "0.7rem" }}>↓</span>}
        </motion.div>
      ))}
    </div>
  );
...
function ServiceCard({ s, t, c }: {
  ...
  return (
    <div style={{ width: "100%", height: CARD_H, perspective: "1200px", cursor: "pointer" }} onClick={() => setFlipped(f => !f)}>
      <div style={{
        ...
        <div style={{
          ...
          backgroundColor: t.bg, border: `1px solid ${t.accent}18`,
          display: "flex", flexDirection: "column", overflow: "hidden",
        }}>
          {/* Image area — fixed height */}
          <div style={{ height: IMG_H, flexShrink: 0, overflow: "hidden" }}>
            {s.i
          ...
          <div style={{ flex: 1, padding: "1.25rem 1.4rem", display: "flex", flexDirection: "column", justifyContent: "space-between", overflow: "hidden" }}>
            <div>
              <p style={{ fontSize: "0.58rem", textTransform: "uppercase", letterSpacing: "0.18em", color: t.accent, fontWeight: 800, marginBottom: "0.5rem", opacity: 0.7, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.sub}</p>
              {/* Title — 1 line max */}
              <h3 style={{ fontSize: "1.25rem", fontWeight: 700, lineHeight: 1.2, marginBottom: "0.7rem", color: t.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{s.t}</h3>
              {/* Description — exactly 2 lines, clamped */}
              <p style={{
                fontSize: "0.82rem", lineHeight: 1.55, color: t.sub,
                display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
              }}>{s.d}</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.62rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.15em", color: t.accent, marginTop: "0.75rem" }}>
              {c.ver_mas} <span style={{ fontSize: "0.75rem" }}>↗</span>
            </div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: "0.58rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.15em", opacity: 0.55, marginBottom: "0.75rem" }}>{c.paquete} {s.num} · {s.sub}</p>
          <h3 style={{ fontSize: "1.5rem", fontWeight: 700, lineHeight: 1.2, marginBottom: "1rem" }}>{s.t}</h3>
          <p style={{ fontSize: "0.84rem", lineHeight: 1.75, opacity: 0.92,
            display: "-webkit-box", WebkitLineClamp: 6, WebkitBoxOrient: "vertical", overflow: "hidden",
          }}>{s.back}</p>
          <div style={{ marginTop: "auto", paddingTop: "1.5rem", fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", opacity: 0.45 }}>{c.volver}</div>
        </div>
      </div>
    </div>
  );
...
function ServicesSwiper({ t, c }: { t: typeof themes.white; c: typeof copy.es }) {
  ...
  const CARD_W = "min(400px, 82vw)";
  const SIDE_W = "min(130px, 22vw)";
    ...
    <div>
      ...
      <div style={{ position: "relative", userSelect: "none", touchAction: "pan-y" }}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", padding: "0.5rem 0 1.5rem", position: "relative", zIndex: 1, minHeight: 420 }}>
          ...
          {/* Prev — only visible, no interaction on mobile */}
          <div onClick={() => goTo(active - 1)} style={{
          ...
          <AnimatePresence mode="wait">
            <motion.div key={`card-${active}`}
              ...
              style={{ width: CARD_W, flexShrink: 0, zIndex: 2, pointerEvents: "auto" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
          {c.servicios.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} style={{
              ...
              backgroundColor: active === i ? t.accent : `${t.accent}30`,
  ...
  return (
    <main style={{ backgroundColor: t.bg, color: t.text, minHeight: "100vh", position: "relative", transition: "background-color 0.5s, color 0.5s" }}>
      ...
      <button onClick={cycleTheme}
        style={{ position: "fixed", bottom: "10.5rem", right: "1.5rem", zIndex: 100, padding: "0.75rem", borderRadius: 999, backgroundColor: t.card, color: t.accent, border: `1px solid ${t.accent}30`, cursor: "pointer", boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
        <Palette size={18} />
      </button>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, backdropFilter: "blur(14px)", backgroundColor: t.navBg, borderBottom: `1px solid ${t.accent}12`, padding: "0.9rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="hidden md:flex items-center" style={{ gap: "1.75rem" }}>
          {([["#problema", c.nav.problema], ["#servicios", c.nav.soluciones], ["#cotizar", c.nav.precios]] as [string,string][]).map(([href, label]) => (
            <a key={href} href={href} style={{ fontSize: "0.68rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: t.sub, textDecoration: "none" }}>{label}</a>
          ))}
          <a href={CALENDLY_LINK} target="_blank"
            style={{ padding: "0.45rem 1.3rem", backgroundColor: t.accent, color: t.bg, fontWeight: 900, fontSize: "0.68rem", textTransform: "uppercase", textDecoration: "none" }}>
            {c.nav.cta}
          </a>
        </div>
        {/* Mobile: just the lang popup trigger */}
        <button onClick={() => setShowPopup(true)} className="md:hidden"
          style={{ display: "flex", alignItems: "center", gap: "0.25rem", padding: "0.3rem 0.8rem", border: `1px solid ${t.accent}30`, borderRadius: 999, fontSize: "0.68rem", fontWeight: 700, color: t.text, backgroundColor: `${t.accent}08`, cursor: "pointer" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "center" }} className="!grid-cols-1 md:!grid-cols-2">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
              <a href={CALENDLY_LINK} target="_blank" style={{ padding: "1rem 2.2rem", backgroundColor: t.accent, color: t.bg, fontWeight: 900, textTransform: "uppercase", fontSize: "0.75rem", letterSpacing: "0.05em", textDecoration: "none" }}>{c.cta1}</a>
              <a href="#servicios" style={{ padding: "1rem 2.2rem", border: `1px solid ${t.accent}`, color: t.accent, fontWeight: 700, textTransform: "uppercase", fontSize: "0.75rem", textDecoration: "none" }}>{c.cta2}</a>
            </div>
          </motion.div>
            ...
            {c.problemas.map(({ q, desc }, i) => {
              ...
              return (
                <motion.div key={i}
                  ...
                  style={{ padding: "1.5rem", borderLeft: `2px solid ${t.accent}35`, backgroundColor: `${t.accent}04`, display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                  {/* Icon inline with text, color of next theme */}
                  <motion.div
      ...
      {/* STATS — label arriba, número abajo, fondo muy transparente */}
        ...
        <div style={{ maxWidth: "52rem", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem", textAlign: "center" }}>
          {c.stats.map(({ label, n }, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              style={{ padding: "2rem 1rem", backgroundColor: `${t.accent}06`, border: `1px solid ${t.accent}10` }}>
      ...
      <section id="cotizar" style={{ padding: "5rem 1.5rem", position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: "38rem", margin: "0 auto", textAlign: "center" }}>
          ...
          <a href={waLink} target="_blank"
            style={{ display: "inline-block", padding: "1rem 2.5rem", backgroundColor: t.accent, color: t.bg, fontWeight: 900, textTransform: "uppercase", fontSize: "0.78rem", letterSpacing: "0.08em", textDecoration: "none" }}>
            {c.cotizar_cta}
          </a>
        </div>
      </section>
        ...
        <div style={{ maxWidth: "68rem", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }} className="!grid-cols-1 md:!grid-cols-2">
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
              <a href={CALENDLY_LINK} target="_blank" style={{ padding: "0.85rem 1.75rem", border: `1px solid ${t.accent}`, color: t.accent, fontWeight: 700, fontSize: "0.78rem", textDecoration: "none" }}>{c.btn_zoom}</a>
